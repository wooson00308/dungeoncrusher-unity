...
public class Skill_old : MonoBehaviour
{
    private Unit _owner;
    private int _skillLevel = 1;
    [SerializeField] private SkillData_old _skillData;
    private float _timeMarker;
    private bool _isInitialized = false;
    private bool _isCooldown;
    public float TimeMarker => _timeMarker;
    public bool IsCooldown => _isCooldown;
    public SkillData_old SkillData => _skillData;
    ...
    public void SetSkillData(SkillData_old skillData)
    {
    ...
    private void OnEnable()
    {
        ...
        {
            GameEventSystem.Instance.Subscribe((int)_skillData.SkillEventType, TryUseEventSkill);
        }
        else
        {
            GameEventSystem.Instance.Subscribe((int)UnitEvents.UnitEvent_UseSkill_Publish_UI, TryUseSkillFromUI);
        }
    }
    ...
    private void OnDisable()
    {
        ...
        {
            // 필살기로 지정될 이벤트 등록 필요
            GameEventSystem.Instance.Unsubscribe((int)UnitEvents.UnitEvent_UseSkill_Publish_UI,
                TryUseSkillFromUI);
        }

        if (_skillData.IsPassiveSkill)
        {
            GameEventSystem.Instance.Unsubscribe((int)_skillData.SkillEventType, TryUseEventSkill);
        }
    }
    ...
    private void UseSkill(Unit user)
    {
        ...
        if (!skillFxObject.TryGetComponent<SkillEffect_old>(out var skillFx)) return;
